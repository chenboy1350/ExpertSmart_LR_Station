...
namespace LR_Station
{
    public partial class History : System.Web.UI.Page
    {
        ...
        private async void GetHistoryInfo_FromAPI()
        {
            try
            {
                AllHistoryInfo dataResult = new AllHistoryInfo();
                string responseBody = "";
                using (HttpClient client = new HttpClient())
                {
                    string RestURL = string.Format("{0}/GetAllHistory/", LR.Properties.Settings.Default.API);
                    var response = await client.GetAsync(RestURL);
                    responseBody = await response.Content.ReadAsStringAsync();
                }
                dataResult = JsonConvert.DeserializeObject<AllHistoryInfo>(responseBody);
                ...
                else
                {
                    ...
                    dtBed1.Columns.Add("end_with", typeof(String));
                    dtBed1.Columns.Add("end_date", typeof(String));
                    dtBed1.Columns.Add("end_efw", typeof(String));
                    dtBed1.Columns.Add("end_agpar", typeof(String));
                    dtBed1.Columns.Add("end_follow", typeof(String));
                    ...
                    dtBed2.Columns.Add("end_with", typeof(String));
                    dtBed2.Columns.Add("end_without_date", typeof(String));
                    dtBed2.Columns.Add("end_deliver_status", typeof(String));
                    ...
                    foreach (var patien in dataResult.HistoryList)
                    {
                        if (patien.end_with.ToString() == "Delivery")
                        {
                            dtBed1.Rows.Add(patien.hn.ToString(), patien.name.ToString(), patien.end_with.ToString(), patien.end_date.ToString(), patien.end_EFW.ToString(), patien.end_AGPAR.ToString(), patien.end_Follow.ToString(), patien.status.ToString(), patien.age.ToString(), patien.gpal.ToString(), patien.ga.ToString(), patien.efw.ToString(), patien.efw_twin.ToString(), patien.pv_dilate.ToString(), patien.pv_effacement.ToString(), patien.pv_membrane.ToString(), patien.pv_station.ToString(), patien.pv_present.ToString(), patien.fhs.ToString(), patien.utene.ToString());
                            foreach (var events in patien.EvenHistorytList)
                            {
                        ...
                        if (patien.end_with.ToString() == "Out of Delivery")
                        {
                            dtBed2.Rows.Add(patien.hn.ToString(), patien.name.ToString(), patien.end_with.ToString(), patien.end_without_date.ToString(), patien.end_deliver_status.ToString(), patien.status.ToString(), patien.age.ToString(), patien.gpal.ToString(), patien.ga.ToString(), patien.efw.ToString(), patien.efw_twin.ToString(), patien.pv_dilate.ToString(), patien.pv_effacement.ToString(), patien.pv_membrane.ToString(), patien.pv_station.ToString(), patien.pv_present.ToString(), patien.fhs.ToString(), patien.utene.ToString());
                            foreach (var events in patien.EvenHistorytList)
                            {
